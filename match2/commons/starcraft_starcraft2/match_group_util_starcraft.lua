...
function StarcraftMatchGroupUtil.matchFromRecord(record)
	...
	local advanceCount = tonumber(Table.extract(match.extradata, 'advancecount'))
	if advanceCount then
		for i = #match.bracketData.advanceSpots + 1, advanceCount do
			local prev = match.bracketData.advanceSpots[i - 1]
			match.bracketData.advanceSpots[i] = prev and Table.copy(prev) or {advance = 'up', type = 'custom'}
		end
	end
...
function StarcraftMatchGroupUtil.populateOpponents(match)
	...
	for _, opponent in ipairs(opponents) do
		...
		opponent.status2 = Table.extract(opponent.extradata, 'score2') and 'S' or nil
