...
function FFAMatchSummary.PlacementCells(props)
	...
	local function Placement(rowIx, group)
		...
		if #match.bracketData.advanceSpots > 0 then
			local spot = match.bracketData.advanceSpots[placement or rowIx]
			bgClass = FFAMatchSummary.getAdvanceClass(spot and spot.advance or 'down')
		else
			bgClass = FFAMatchSummary.getPlacementClass(placement or rowIx)
		end

		local parts = {
			Ordinal._ordinal(rowIx),
			#group > 1 and '&#8203;–&#8203;' .. Ordinal._ordinal(rowIx + #group - 1) or '',
		}
	...
	local groups = Array.groupBy(props.opponentIxs, function(ix) return match.opponents[ix].placement or '' end)

	-- Loop through placement groups and draw cells
	local cells = {}
	local rowIx = 1
	for placement, group in ipairs(groups) do
		if placement == '' then
			for ix, opponentIx in ipairs(group) do
				table.insert(cells, Placement(rowIx + ix, {opponentIx}))
			end
		else
...
function FFAMatchSummary.getOpponentBgClass(opponent, match)
	if #match.bracketData.advanceSpots > 0 then
		local spot = match.bracketData.advanceSpots[opponent.placement]
		return FFAMatchSummary.getAdvanceClass(spot and spot.advance or 'down')
	else
		return FFAMatchSummary.getPlacementClass(opponent.placement)
	end
end
...
function FFAMatchSummary.getAdvanceClass(advance)
	return 'bg-' .. advance
end
